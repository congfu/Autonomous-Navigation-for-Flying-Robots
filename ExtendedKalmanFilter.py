...
class UserCode:
    ...
    def calculatePredictMeasurementJacobian(self, x, marker_position_world, marker_yaw_world):
		...
		#H11 = -math.cos(marker_yaw_world)
		#H12 = -math.sin(marker_yaw_world)
		#H13 = -(x_m - x[0])*math.sin(marker_yaw_world) + (y_m-x[1])*math.cos(marker_yaw_world)
		#H21 = math.sin(marker_yaw_world)
		#H22 = -math.cos(marker_yaw_world)
		#H23 = -(x_m - x[0])*math.cos(marker_yaw_world) - (y_m-x[1])*math.sin(marker_yaw_world)
        return np.array([[H11, H12, H13], [H21, H22, H23], [0, 0, -1]])  #np.zeros((3,3))
